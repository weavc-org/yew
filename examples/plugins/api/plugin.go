...
// Plugin variable that implements github.com/weavc/yew/v3/pkg/plugin.Plugin
...
func (p *APIPlugin) Manifest() pkg.Manifest {
	return pkg.Manifest{Namespace: "examples.api", Description: "Api plugin"}
}

// Register is used to initialize & setup the plugin
func (p *APIPlugin) Setup(m pkg.Handler) error {
...
func (p *APIPlugin) RegisterRoutes(mux *http.ServeMux) {
	mux.HandleFunc("/api", func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(200)
		json.NewEncoder(w).Encode(map[string]string{"status": "success", "payload": "hello world"})
	})
}
